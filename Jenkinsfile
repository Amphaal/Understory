                ...
                stage('MinGW') {
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'jenkins-sentry-auth-token', variable: 'SENTRY_AUTH_TOKEN')]) {
                            sh 'sentry-cli --auth-token $SENTRY_AUTH_TOKEN releases new -p understory $(cat _pkg/version)'
                            sh 'sentry-cli --auth-token $SENTRY_AUTH_TOKEN releases set-commits --auto $(cat _pkg/version)'
                            sh 'sentry-cli --auth-token $SENTRY_AUTH_TOKEN upload-dif --wait _pkg/out/UnderStory.exe _pkg/out/UnderStory.pdb'
                        }
