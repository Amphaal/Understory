...
class Scroller : public Magnum::GL::AbstractShaderProgram {
 public:
    typedef Magnum::GL::Attribute<0, Magnum::Vector2> Position;
    struct Vertex {
        Magnum::Vector2 position;
    };
    explicit Scroller(Magnum::NoCreateT): Magnum::GL::AbstractShaderProgram{Magnum::NoCreate} {}

    explicit Scroller(const Magnum::Utility::Resource &rs) {
            ...
            vert.addSource(rs.get("Scroller.vert"));
            frag.addSource(rs.get("Scroller.frag"));
        ...
        Scroller& setInnerRect(const Magnum::Vector4& borderRect) {
            setUniform(_innerRectUniform, borderRect);
            return *this;
        }

        Scroller& setProjectionMatrix(const Magnum::Matrix3& projectionMatrix) {
