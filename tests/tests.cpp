...
TEST_CASE("Version comparaison", "[update checker]") {
    ...
    REQUIRE_FALSE(UpdateChecker::_isVersionNewerThanLocal(APP_CURRENT_VERSION));

    std::string test_localVersion("0.5.0");
    REQUIRE_FALSE(UpdateChecker::_isVersionNewerThanLocal("0.5.0", test_localVersion));
    REQUIRE(UpdateChecker::_isVersionNewerThanLocal("0.5.1", test_localVersion));
    REQUIRE(UpdateChecker::_isVersionNewerThanLocal("0.5.10", test_localVersion));
    REQUIRE_FALSE(UpdateChecker::_isVersionNewerThanLocal("0.4.10", test_localVersion));
    REQUIRE(UpdateChecker::_isVersionNewerThanLocal("1.0", test_localVersion));
}
